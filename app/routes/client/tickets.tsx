...
import { getTickets, type Ticket as TicketItem, type TicketStatus } from "~/services/tickets";
...
export function meta() {
  return [{ title: "My Tickets — Buhata" }];
}
      ...
      <div className="p-6 space-y-4">
        {fetchError && (
          <div className="rounded-lg border border-terracotta/20 bg-terracotta/8 px-4 py-3 text-xs text-terracotta">
            Could not load tickets. Check that the backend server is running.
          </div>
        )}
                ...
                {filtered.map((ticket) => (
                  ...
                  >
                    <td className="px-6 py-3.5">
                      ...
                      {ticket.description && (
                        <p className="mt-0.5 text-xs text-cream/35 truncate max-w-xs">
                          {ticket.description}
                        </p>
                      )}
                    </td>
