...
export async function clientLoader(): Promise<LoaderData | Response> {
  ...
  try {
    const result = await getUpgrades({ page_size: 100 });
...
export function meta() {
  return [{ title: "Upgrades — Buhata Admin" }];
}
                ...
                {filtered.map((upgrade) => (
                  ...
                  >
                    <td className="px-6 py-3.5">
                      <Link
                        to={`/admin/upgrades/${upgrade.id}`}
                        className="group"
                      >
                        <p className="text-sm font-medium text-cream group-hover:text-amber transition-colors">
                          {upgrade.description.length > 60
                            ? upgrade.description.slice(0, 60) + "…"
                            : upgrade.description}
                        </p>
                        <p className="mt-0.5 text-xs text-cream/30 font-mono">
                          {upgrade.client_id.slice(0, 8)}
                        </p>
                      </Link>
                    </td>
                    <td className="px-6 py-3.5 text-sm text-cream/40 hidden md:table-cell capitalize">
                      {upgrade.complexity_class ?? "—"}
                    </td>
                    <td className="px-6 py-3.5 text-sm text-cream/70">
